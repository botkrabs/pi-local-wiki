...
- `local_wiki_get(title, section=..., full=...)` — offline Wikipedia ZIM, ~35 ms, frozen at the ZIM's download date (stated in the local_wiki README)
- `local_wiki_search(query)` — phrase search over article text; ranked titles + snippets, no exact title needed
...
- **Title unknown but guessable** → guess via `get`; on "not found", retry with one of the suggested related titles (one round-trip, no web needed).
